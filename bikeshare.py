...
def get_filters():
  """
    ...
    while True:
        city = choice('For what city(ies) do you want do select data: '
                      'New York City, Chicago or Washington? Use commas '
                      'to list the names.>', CITY_DATA.keys()
        month = choice('From January to June, for what month(s) do you '
                       "want do filter data? Use commas to list the names.>',
                       months)
        day = choice('For what weekday(s) do you want do filter bikeshare '
                     'data? Use commas to list the names.>', weekdays)

        # confirm the user input
        confirmation = choice('Please confirm that you would like to apply '
                              'the following filter(s) to the bikeshare data.'
                              'City(ies): {} Month(s): {} Weekday(s)'
                              ': {}[y] Yes [n] No>'
...
def load_data(city, month, day):
    ...
    print('The program is loading the data for the filters of your choice.')
...
def user_stats(df, city):
    """Display statistics on bikeshare users in the given city"""
